...
use org\fla\seminar\SeminarConfigurator;
	...
	if (($index = $config->findIndex($name)) === false) {
	...
	$showqr = (count($tickets) > 1);

#	if (!$showqr) {
#		header("Refresh: 0; url={$tickets[0]['url']->toString()}");
#	}
...
<?php if (false) { ?>
<meta http-equiv="refresh" content="0; url=<?php echo $tickets[0]['url']->toString(); ?>" />
<?php } ?>
...
<meta property="og:url" content="<?php echo "{$url->toString(Url::TOK_HOST)}/n/{$name}"; ?>" />
...
document.addEventListener('DOMContentLoaded', event => {
	document.querySelector("img.qr").addEventListener('load', (event) => {
		location.href = "<?php echo $tickets[0]['url']->toString(); ?>";
	});
});
	...
	<li style="<?php echo ($pname == $ticket['name'])? '': 'grid-column-start: 1;'; ?>">
		<a href="<?php echo $ticket['url']->toString(); ?>">
			<ul class="qrcode">
				...
				<li>
					<img class="qr" src="/i/<?php echo $ticket['id']; ?>" loading="lazy" />
				...
				<li>
					<p>
						...
						<br />
						<span><a href="<?php echo $ticket['url']->toString(); ?>">กดหรือแสกน QR โค้ดลงทะเบียนเข้างาน</a></span>
					</p>
				</li>
			</ul>
		</a>
