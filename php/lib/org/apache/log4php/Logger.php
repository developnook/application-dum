 ...
 * @version	   SVN: $Id: Logger.php,v 1.1 2013/12/19 13:55:40 moserv Exp $
...
class Logger {
	...
	public function log(LoggerLevel $level, $message, $throwable = null) {
		...
		if (isset($traces[1])) {
			$filename = basename($traces[1]['file']);
			$line = $traces[1]['line'];
			$callee = "$filename:$line";
		}
		else
			$callee = 'none';

		if (isset($traces[2])) {
			$filename = basename($traces[2]['file']);
			$line = $traces[2]['line'];
		...
		if($this->isEnabledFor($level)) {
			$this->forcedLog($this->fqcn, $throwable, $level, "$caller/$callee - $message");
		}
	}
