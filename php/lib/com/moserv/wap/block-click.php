	...
	protected function doExecute() {
		...
		switch ($this->telcoId) {
			case WapMedia::telco_dtc:
				return ($this->serviceId != 601 && $this->serviceId != 0)? ClickBlocker::blk_temp: ClickBlocker::blk_none;
			break;

			case WapMedia::telco_tmv:
			case WapMedia::telco_rmv:
				return (preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2} 0[0-6]/', $this->timestamp))? ClickBlocker::blk_temp: ClickBlocker::blk_none;
			break;
		}
...
<<<sql
			...
			from wap.block_msisdn bm
				left join wap.block_issue bi using (block_issue_id)
				left join wap.block_issue_aff bia on bia.block_issue_id = bi.block_issue_id and bia.affiliate_id = ?
			where bm.msisdn = ?
			and bm.enabled = 1
			and coalesce(bi.enabled, 1) <> 0
			and coalesce(bia.enabled, 1) <> 0
sql
		);
...
<<<sql
			...
			where msisdn = ?
			and enabled = 0
			and unregister_timestamp >= date_sub(current_timestamp(), interval 45 day)
sql
		);
...
class QuitSameDayClickBlocker extends LimitClickBlocker {
...
<<<sql
			...
			where msisdn = ?
			and enabled = 0
			and date(register_timestamp) = date(unregister_timestamp)
			and unregister_timestamp >= date_sub(current_timestamp(), interval 90 day)
sql
		);
		...
		return ($blocked)? ClickBlocker::blk_quit_sday: ClickBlocker::blk_none;
	}
}
...
//		}
		...
		if (
			...
			$this->msisdn == '66622944908'	||	// ae ais PostPaid
			$this->msisdn == '66851455817'	||	// ae ais PrePaid
//			$this->msisdn == '66871276524'	||	// p alex ais
			$this->msisdn == '66624610850'	||	// Nook AIS TEST
			$this->msisdn == '66927605872'	||	// Tong AIS TEST
			...
			$this->msisdn == '66994541442'	||	// True K.Da whitelist

			$this->msisdn == '66940438266'	||	// nook whitelist
			$this->msisdn == '66942218898'	||	// nook whitelist
			$this->msisdn == '66954911485'	||	// nook whitelist
			$this->msisdn == '66902904079'	||	// nook whitelist
			$this->msisdn == '66940438266'	||	// nook whitelist
			$this->msisdn == '66841103835'	||	// p-dum
			$this->msisdn == '66954816309'	||	// tong private
			$this->msisdn == '66855217285'	||	// tong true new
			$this->msisdn == '66809478184'	||	// tong true new
			$this->msisdn == '66968463881'	||	// tong true new
			$this->msisdn == '66836806925'	||	// tong true new
			$this->msisdn == '66972012470'	||	// tong true new
			$this->msisdn == '66644584719'	||	// tong true new
			...
			$this->msisdn == '66809389307'	||	// Tortong True-H Prepaid 28 March 2021
			$this->msisdn == '66963281378'	||	// Tortong True-H Prepaid 8 April 2021
			
			$this->msisdn == '66937960785'	||	// tmh whitelist
			$this->msisdn == '66891006619'	||	// tmh whitelist
			$this->msisdn == '66937960793'	||	// tmh whitelist
			$this->msisdn == '66830412312'	||	// tmh whitelist
			$this->msisdn == '66937960870'	||	// tmh whitelist
			$this->msisdn == '66937960871'	||	// tmh whitelist
			$this->msisdn == '66937960872'	||	// tmh whitelist
			$this->msisdn == '66937960875'	||	// tmh whitelist
			$this->msisdn == '66937960877'	||	// tmh whitelist
			$this->msisdn == '66937960879'	||	// tmh whitelist
			$this->msisdn == '66937960883'	||	// tmh whitelist
			$this->msisdn == '66937960884'	||	// tmh whitelist
			$this->msisdn == '66937960885'	||	// tmh whitelist
			$this->msisdn == '66937960886'	||	// tmh whitelist
			$this->msisdn == '66937960891'	||	// tmh whitelist
			$this->msisdn == '66937960892'	||	// tmh whitelist
			$this->msisdn == '66645414901'	||	// tmh whitelist
			$this->msisdn == '66970286619'	||	// tmh whitelist
			$this->msisdn == '66968385052'	||	// tmh whitelist
			$this->msisdn == '66863701309'	||	// tmh whitelist
			$this->msisdn == '66645416106'	||	// tmh whitelist
			$this->msisdn == '66866299868'	||	// tmh whitelist
			$this->msisdn == '66865760020'	||	// tmh whitelist
			$this->msisdn == '66658646940'	||	// tmh whitelist
			$this->msisdn == '66642024560'		// tmh whitelist
		) {
			return ClickBlocker::blk_none;
		}
...
//		$blocker->setLimit(10);
		...
		$blocker = new QuitSameDayClickBlocker($this->session);
		$blocker->setMsisdn($this->msisdn);
		$blocker->setTelcoId($this->telcoId);
		$blocker->setAffiliateId($this->affiliateId);
		$blocker->setLimit(1);
		...
		if (($code = $blocker->execute(false)) != ClickBlocker::blk_none) {
			Logger::$logger->info("blocked [{$this->msisdn}] by Quit Same Day");
