...
#				}
	...
	protected function checkHitBlocker() {
		...
		if (($code = $blocker->execute()) != 0) {
			$pixee = new Pixee($this->session);
			$pixee->setAffiliateId($this->affiliateId);
			list($pixeeId, $pixeeFiringId, $url) = $pixee->execute(false);
...
#		echo "File Not Found: " . $url->toString();
	...
	public function gotoRedirect($url) {

		$xurl = htmlspecialchars($url);
		Logger::$logger->info("gotoRedirect to url => {$url}");
		echo "<?xml version=\"1.0\"?>\n<hawhaw><deck title=\"Redirect\" redirection=\"0; URL={$xurl};proxy=no\" /></hawhaw>";
...
#		}
		...
		if ($telcoId == WapMedia::telco_unk || $telcoId == WapMedia::telco_tmv || $telcoId == WapMedia::telco_rmv) {
...
#			$blocker = new WapBlocker($this->session);
			...
			if (($code = $blocker->execute()) != 0) {
				...
				$pixee = new Pixee($this->session);
				$pixee->setAffiliateId($this->affiliateId);
				list($pixeeId, $pixeeFiringId, $url) = $pixee->execute(true);
