...
#  define("HAX_RAW_HTML_ALLOWED", false);
	define("HAX_RAW_HTML_ALLOWED", true);
		...
		case "DECK": {
			if ($pos == HAX_PARSE_BEGIN) {
				...
				if (isset($attrs["REDIRECTION"])) {
					...
					if (($redirection[0] < 0) || ($redirection[0] > 3600))
					...
					// redirect directly to target (needed for HawTags)
						if ($redirection[0] == 0) {
							...
							header('location: '.substr($redirection[1], 4));
							exit;

#							$codeUrl = new Url(_GET['code']);
#							$aocUrl = new Url(substr($redirection[1], 4));

#							$safeUrl = new Url($codeUrl->toString());
#							$safeUrl->setPort(80);
#							$safeUrl->setPath('/service/index.php');
#							$safeUrl->setParams(null);
#							$safeUrl->setParam('p', $codeUrl->getParam('p'));
#							$safeUrl->setParam('_', '1');
#
#							$landing = new WapLanding($session);
#							$landing->register();

#							$landing->addConfig($safeUrl->toString(), Redirector::rd_chin);
#							$landing->addConfig($aocUrl->toString(), Redirector::rd_href);

#							$landingUrl = new Url($landing->getUrl());
#							$landingUrl->redirect();
						}
			...
			if ($pos == HAX_PARSE_BEGIN) {
				...
				if (isset($attrs["PROXY"]) && (strtolower($attrs["PROXY"]) == "no")) {
					...
					if ($attrs["METHOD"] == "post")
						$form = new HAW_form($attrs["ACTION"], HAW_METHOD_POST);
					else
					...
					if ($attrs["METHOD"] == "post")
						$form = new HAW_form($_SERVER['SCRIPT_NAME'], HAW_METHOD_POST);
					else
	...
	if (preg_match('/image\\.exe\\.php/', $src_wbmp)) {
		$__prefix = '';
	}
	else {
		$__host = preg_replace("/^m\\./", "static.", (new Url())->getHost());
		$__prefix = "//{$__host}";
	}

        if (isset($attrs["BMP"]))
          $image = new HAW_image("{$__prefix}{$src_wbmp}", "{$__prefix}{$src_html}", $alt, $attrs["BMP"]);
        else
          $image = new HAW_image("{$__prefix}{$src_wbmp}", "{$__prefix}{$src_html}", $alt);

        if (isset($attrs["HTML"]) && isset($attrs["HTML_WIDTH"])) {
#		if (preg_match('/^([0-9]+)(%)?$/', $attrs["HTML_WIDTH"], $group) && !empty($group[2]))
#			$image->set_html_width_percentage($group[1]);
#		else
...
#  ini_set('session.use_cookies', 0);
  ...
  else
  {
    ...
    if (function_exists('session_register')) session_register('img_ml');
  }
}
...
#$requester = new HttpProxyRequester();
#$requester->setUrl($remote_url);
#$requester->execute();
...
$curl = new Url($remote_url);
$localFilePath = "{$_SERVER['DOCUMENT_ROOT']}{$curl->getPath()}";
...
if (file_exists($localFilePath)) {
	...
	$serverValues['HTTP_USER_AGENT'] = HttpClient::USER_AGENT;

	$json = json_encode($serverValues);
	$shellCmd = "/usr/local/php/bin/php /usr/project/mpfc/com/moserv/util/shell-call.php '{$localFilePath}' '{$json}' '{$remote_url}'";
	$data = shell_exec($shellCmd);
	Logger::$logger->info("fetch: internal-require => {$localFilePath}");
}
else {
#	$data = file_get_contents($remote_url);
#	Logger::$logger->info("fetch: http-request => {$remote_url}");


	$http = new HttpClient();
	$http->getHeaders()->add('User-Agent', HttpClient::USER_AGENT);
#	$http->getHeaders()->add('Content-Type', 'application/json; charset=utf-8');
#	$http->setMethod(HttpClient::MTHD_POST);
	$http->setMethod(HttpClient::MTHD_GET);
	$http->setUrl($remote_url);

	$http->execute();
	$data = $http->getResponse()->getContent();
}


#Logger::$logger->info("minfo: $data");


if ((!empty($http_response_header) && preg_match('/404/', $http_response_header[0])) || empty($data)) {

	header('HTTP/1.0 404 Not Found');
	header('Status: 404 Not Found');
	header('Content-Type: text/html');

	$codeUrl = new Url($_REQUEST['code']);
	$codeUrl->setPath(str_replace('.xml.php', '.php', $codeUrl->getPath()));

	echo "<html><head><title>404 Not Found</title></head><body><h1>Not Found</h1><p>The requested URL {$codeUrl->toString(Url::TOK_PATH)} was not found on this server.</p></body></html>";

	exit;
}


#Logger::$logger->info("url => $remote_url");
#Logger::$logger->info("content => $data");
/*

$__php = '/usr/local/php/bin/php -r';
$__url = parse_url($remote_url);
$__query = $__url['query'];
$__filepath = "{$_SERVER['DOCUMENT_ROOT']}{$__url['path']}";

$__cmd = "$__php ' parse_str(\"{$__query}\", \$_REQUEST); include(\"{$__filepath}\"); '";


$data = shell_exec($__cmd);
*/
